...
toInt = function (value, radix) {
    return parseInt(value, radix ? radix : 10);
},
...
ext(Dxf.prototype, {
    ...
    getIntValue: function (code, def, radix) {
        if (!this.hasValue(code)) {
            return def;
        }
        return toInt(this.values[code], radix);
    },
    ...
    processDXFGroup: function () {
        ...
        if (groupCode === 0 || groupCode === 9) {
            ...
            var color = self.getIntValue(62, 256),
                ...
                handle = self.getIntValue(5, -1, 16),
    ...
    addPolyline: function () {
        ...
        if (self.currentObjectType === 'LWPOLYLINE') {
            for (i = 0; i < vertices.length / 4/*self.maxVertices*/; i++) {
